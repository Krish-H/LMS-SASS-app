...
export const getAllCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
  ...
  // If user is logged in, fetch bookmarks
  if (!userId) {
...
export const getUserCompanions = async (userId: string) => {
  ...
  // Fetch companions authored by the user
  ...
  // Extract the list of bookmarked companion IDs
  const bookmarkedIds = bookmarksData?.map((b) => b.companion_id) ?? [];

  // Add bookmarked property to each companion
  const data = companionsData.map((companion) => ({
    ...companion,
